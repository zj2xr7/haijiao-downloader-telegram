...
async def handle_url_or_id_input(
    ...
    """Processes user message containing URL or resource ID."""
    ...
    try:
        ...
        # Not a valid target URL or ID; ignore or prompt if /dl was explicitly used
...
async def handle_author_download_callback(callback: CallbackQuery, pipeline: PipelineManager):
    """Handles author batch download triggers from inline buttons."""
    # Data format: author_dl:<author_id>:<start>-<end>
    ...
    # Launch background batch pipeline
...
async def handle_author_custom_page(callback: CallbackQuery, state: FSMContext):
    """Prompts user to enter custom page numbers."""
...
async def handle_custom_pages_input(
    ...
    """Parses user-entered page numbers and starts author batch."""
